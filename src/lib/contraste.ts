...
import { inTransaction, queryAll, queryOne } from './sqlite';
import { clavesDeNorma, extraerCitas, type Cita } from './citas';
import {
  UMBRAL_CONTENCION,
  UMBRAL_JACCARD,
  clasificar,
...
import type { NormRecord, Risk } from './types';

/**
 * Etapas 5 y 6: validación de las citas normativas contra el catálogo y
 * comparación del documento con el repositorio.
 *
 * Vive aquí y no en su ruta porque tiene dos llamadores: la ruta, para
 * ejecutarlo por separado, y la evaluación, que lo corre al terminar. Antes
 * dependía de que alguien pulsara un botón, y un contraste que nadie lanza es
 * una etapa que no existe.
 *
 * Ninguna de las dos etapas usa modelos de lenguaje: el texto no sale del
 * equipo.
 */

export const ETAPAS_POR_OMISION = ['normativa', 'similitud'] as const;

export interface ResultadoDelContraste {
  etapas: string[];
  resumen: Record<string, unknown>;
  hallazgos: number;
}

/** Error con un mensaje y un estado pensados para devolverse tal cual. */
export class ErrorDeContraste extends Error {
  constructor(
    ...
    super(message);
    this.name = 'ErrorDeContraste';
  }
}

/** Hallazgo listo para insertar, sin los campos que fija esta librería. */
...
/**
 * Ejecuta el contraste sobre un documento y guarda sus hallazgos.
 *
 * Los hallazgos de cada etapa se reemplazan en cada ejecución, de modo que
 * repetir el contraste no acumula duplicados ni toca los de la evaluación.
 */
export function ejecutarContraste(
  db: DatabaseSync,
  documentId: string,
  etapas: string[] = [...ETAPAS_POR_OMISION],
): ResultadoDelContraste {
  ...
  if (!documento) {
    throw new ErrorDeContraste('El documento no existe.', 404);
  }
  ...
  if (!contenido) {
    throw new ErrorDeContraste(
      'El documento no tiene texto extraído; no hay nada que contrastar.',
      409,
    );
  }
  ...
  inTransaction(db, () => {
    ...
    for (const etapa of etapas) borrar.run(documentId, etapa);
  ...
  return { etapas, resumen, hallazgos: hallazgos.length };
}
